...
% 打开文本文件
fid_gray = fopen('./img_dst_gray/img_gray_fpga.txt', 'r'); % 以读的方式打开文件
...
imwrite(gray_normalization, './img_dst_gray/img_gray_fpga.jpeg'); % 图像句柄，要写入的磁盘路径
...
imwrite(img_dst_gray_fpga, './img_dst_gray/img_gray_fpga.jpeg'); % 图像句柄，要写入的磁盘路径
[dst_pixel_cnt, dst_pixel] = imhist(img_dst_gray_fpga);
figure;
title('fpga均衡化后的灰度直方图');
